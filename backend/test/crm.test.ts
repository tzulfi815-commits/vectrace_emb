...
import { computeStage, CrmService, inDateRange, isCountedCall, type Session } from "../src/services/crm.js";
import { createSeededRepositories } from "../src/repositories/seed.js";

const admin: Session = { token: "test", userId: "usr-admin", name: "Ayesha Khan", role: "Admin" };
const caller: Session = { token: "test", userId: "usr-caller-1", name: "Hamza Ali", role: "Caller" };
...
describe("StitchCRM business rules", () => {
  ...
  it("prevents callers from marking leads won directly", async () => { await expect(service().updateLead(caller, "lead-1", { status: "Closed - Won" })).rejects.toThrow("Forbidden"); });
  it("prevents callers from logging calls on another owner's lead", async () => { await expect(service().logCall(caller, "lead-4", 60, "No access")).rejects.toThrow("Forbidden"); });
});
